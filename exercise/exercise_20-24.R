...
# first part of figure: trends in physical activity by ascvd status
...
pB <- make_exercise_panel("strength", show_legend = FALSE, tag = "B", ymax = 0.25)
pC <- make_exercise_panel("aerobic", show_legend = FALSE, tag = "C", ymax = 0.5)
pD <- make_exercise_panel("bothexercise", show_legend = TRUE, tag = "D", ymax = 0.5)

# second part of figure: trends in physical activity by race, income, and age
...
pE <- make_panel(results_income, "income_group", "noexercise", FALSE, "E", "Family Income", ymax = 0.75)
pF <- make_panel(results_income, "income_group", "strength", FALSE, "F", "Family Income", ymax = 0.25)
pG <- make_panel(results_income, "income_group", "aerobic", FALSE, "G", "Family Income", ymax = 0.5)
pH <- make_panel(results_income, "income_group", "bothexercise", TRUE, "H", "Family Income", ymax = 0.5)

pI <- make_panel(results_age, "age_group", "noexercise", FALSE, "I", "Age", ymax = 0.75)
pJ <- make_panel(results_age, "age_group", "strength", FALSE, "J", "Age", ymax = 0.25)
pK <- make_panel(results_age, "age_group", "aerobic", FALSE, "K", "Age", ymax = 0.5)
pL <- make_panel(results_age, "age_group", "bothexercise", TRUE, "L", "Age", ymax = 0.5)

combined <- (pA | pB | pC | pD) / (pE | pF | pG | pH) / (pI | pJ | pK | pL)

ggsave('combined_fig_exercise_race_income_age_120825_v1.jpeg', combined, device = jpeg, width = 16.5, height = 14.5)
