...
# figure 1 average sleep duration by CVD status
df_plot <- df %>%
...
df_combined <- bind_rows(df_plot, df_overall) %>%
  mutate(cvd_status = factor(cvd_status, levels = c("Overall", "No CVD", "CVD")))
...
for(yr in c(2020, 2022, 2024)) {
  for(status in c("Overall", "No CVD", "CVD")) {
...
quantiles_df <- bind_rows(quantiles_list) %>%
  mutate(
    cvd_status = factor(cvd_status, levels = c("Overall", "No CVD", "CVD")),
    year_fct = factor(year)
  )
...
ggsave('/sleep/fig1_sleep_overall_111225_v1.pdf', fig1, device = 'pdf', width = 15, height = 12)
...
pB <- make_plot(results_race, "race_group", "enoughsleep",
                "% reporting \u2265 7 hrs of sleep",
                "", "B", show_legend = TRUE, legend_title = "Race/Ethnicity", y_min = 0.4, y_max = 0.8)
pC <- make_plot(results_income, "income_group", "shortsleep",
                "% reporting < 7 hrs of sleep",
                "", "C", show_legend = FALSE, legend_title = "Family Income", y_min = 0.2, y_max = 0.6)
pD <- make_plot(results_income, "income_group", "enoughsleep",
                "% reporting \u2265 7 hrs of sleep",
                "", "D", show_legend = TRUE, legend_title = "Family Income", y_min = 0.4, y_max = 0.8)
pE <- make_plot(results_age, "age_group", "shortsleep",
                "% reporting < 7 hrs of sleep",
                "", "E", show_legend = FALSE, legend_title = "Age", y_min = 0.2, y_max = 0.6)
pF <- make_plot(results_age, "age_group", "enoughsleep",
                "% reporting \u2265 7 hrs of sleep",
                "", "F", show_legend = TRUE, legend_title = "Age", y_min = 0.4, y_max = 0.8)

fig2 <- (pA | pB) /
  (pC | pD) /
  (pE | pF)

ggsave('/sleep/fig2_short_enough_sleep_111225_v1.jpeg', fig2, device = jpeg, width = 15, height = 12)
