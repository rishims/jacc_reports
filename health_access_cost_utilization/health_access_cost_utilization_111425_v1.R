...
# Data source is the National Health Interview Survey (https://www.cdc.gov/nchs/nhis/index.html)
# 04 November 2025
...
# first part of figure: trends in access, affordability, and utilization by ascvd status and year
...
make_panel <- function(outcome_name, show_legend, tag) {
  ...
  if (show_legend) {
    p <- p + theme(legend.title.position = "top", legend.direction = "horizontal", legend.position = "bottom")
  } else {
...
pA_fig1 <- make_panel("noinsurance", show_legend = FALSE, tag = "A")
pD_fig1 <- make_panel("noplace", show_legend = TRUE, tag = "D")

pA_fig2 <- make_panel("crn_barrier", show_legend = FALSE, tag = "A")
pD_fig2 <- make_panel("badcarecost", show_legend = TRUE, tag = "D")

pA_fig3 <- make_panel_t("telehealth", show_legend = FALSE, tag = "A")
pD_fig3 <- make_panel("novisit", show_legend = TRUE, tag = "D")

# second part of figure: access, affordability, and utilization by race and income among the whole population
...
make_panel_sub_t <- function(data, group_var, outcome_name, show_legend, tag, legend_title) {
  ...
  if (show_legend) {
    p <- p + theme(legend.title.position = "top", legend.direction = "horiozntal", legend.position = "bottom")
  } else {
    p <- p + theme(legend.position = "none")
  }
...
pB_race_fig1 <- make_panel_sub(results_race_final, "race_group", "noinsurance", FALSE, "B", "Race/Ethnicity")
pE_race_fig1 <- make_panel_sub(results_race_final, "race_group", "noplace", TRUE, "E", "Race/Ethnicity")
pC_income_fig1 <- make_panel_sub(results_income_final, "income_group", "noinsurance", FALSE, "C", "Family Income")
pF_income_fig1 <- make_panel_sub(results_income_final, "income_group", "noplace", TRUE, "F", "Family Income")

figure1 <- (pA_fig1 | pB_race_fig1 | pC_income_fig1) / (pD_fig1 | pE_race_fig1 | pF_income_fig1)

pB_race_fig2 <- make_panel_sub(results_race_final, "race_group", "crn_barrier", FALSE, "B", "Race/Ethnicity")
pE_race_fig2 <- make_panel_sub(results_race_final, "race_group", "badcarecost", TRUE, "E", "Race/Ethnicity")
pC_income_fig2 <- make_panel_sub(results_income_final, "income_group", "crn_barrier", FALSE, "C", "Family Income")
pF_income_fig2 <- make_panel_sub(results_income_final, "income_group", "badcarecost", TRUE, "F", "Family Income")

figure2 <- (pA_fig2 | pB_race_fig2 | pC_income_fig2) / (pD_fig2 | pE_race_fig2 | pF_income_fig2)

pB_race_fig3 <- make_panel_sub_t(results_race_final, "race_group", "telehealth", FALSE, "B", "Race/Ethnicity")
pE_race_fig3 <- make_panel_sub(results_race_final, "race_group", "novisit", TRUE, "E", "Race/Ethnicity")
pC_income_fig3 <- make_panel_sub_t(results_income_final, "income_group", "telehealth", FALSE, "C", "Family Income")
pF_income_fig3 <- make_panel_sub(results_income_final, "income_group", "novisit", TRUE, "F", "Family Income")

figure3 <- (pA_fig3 | pB_race_fig3 | pC_income_fig3) / (pD_fig3 | pE_race_fig3 | pF_income_fig3)

ggsave('combined_fig_access_race_income_121025_v2.pdf', figure1, device = pdf, width = 20, height = 14)
ggsave('combined_fig_cost_race_income_121025_v2.pdf', figure2, device = pdf, width = 20, height = 14)
ggsave('combined_fig_utilization_race_income_121025_v2.pdf', figure3, device = pdf, width = 20, height = 14)
