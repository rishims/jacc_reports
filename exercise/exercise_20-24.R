...
df$strength <- ifelse(df$pa18aerstr %in% c(2, 4), 1, 
                      ifelse(df$pa18aerstr %in% c(1, 3), 0, NA))

df$aerobic <- ifelse(df$pa18aerstr %in% c(3, 4), 1, 
                     ifelse(df$pa18aerstr %in% c(1, 2), 0, NA))
...
make_exercise_panel <- function(outcome_name, show_legend, tag, ymax = 0.5) {
  p <- ggplot(
...
pA <- make_exercise_panel("noexercise", show_legend = FALSE, tag = "A", ymax = 0.8)
pB <- make_exercise_panel("strength", show_legend = TRUE, tag = "B")
pC <- make_exercise_panel("aerobic", show_legend = FALSE, tag = "C")
pD <- make_exercise_panel("bothexercise", show_legend = FALSE, tag = "D")
...
pA <- make_panel(results_income, "income_group", "noexercise", FALSE, "A", "Family Income")
pB <- make_panel(results_income, "income_group", "strength", FALSE, "B", "Family Income")
pC <- make_panel(results_income, "income_group", "aerobic", FALSE, "C", "Family Income")
pD <- make_panel(results_income, "income_group", "bothexercise", TRUE, "D", "Family Income")

pE <- make_panel(results_age, "age_group", "noexercise", FALSE, "E", "Age")
pF <- make_panel(results_age, "age_group", "strength", FALSE, "F", "Age")
pG <- make_panel(results_age, "age_group", "aerobic", FALSE, "G", "Age")
pH <- make_panel(results_age, "age_group", "bothexercise", TRUE, "H", "Age")

figure_2 <- (pA | pB | pC | pD) / (pE | pF | pG | pH)

ggsave('fig2_exercise_race_income_age_111825_v1.jpeg', figure_2, device = jpeg, width = 15, height = 12)
