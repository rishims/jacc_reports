...
# for this analysis, we do not need to divide by years because we are not pooling, but rather looking at each year individually
...
# first part of figure: average sleep duration by ASCVD status
df_plot <- df %>%
...
write.csv(summary_stats, "sleep_cvd_summary_stats.csv", row.names = FALSE)
...
write.csv(results_race, file = "ratefile_sleep_race_111225.csv", row.names = FALSE)
...
write.csv(results_income, file = "ratefile_sleep_income_111225.csv", row.names = FALSE)
...
write.csv(results_age, file = "ratefile_sleep_age_111225.csv", row.names = FALSE)
...
pA <- make_plot(results_race, "race_group", "shortsleep",
                ...
                "", "B", show_legend = FALSE, legend_title = "Race/Ethnicity", y_min = 0.2, y_max = 0.6)
pB <- make_plot(results_race, "race_group", "longsleep",
                "% Reporting > 9 hrs of sleep",
                "", "C", show_legend = TRUE, legend_title = "Race/Ethnicity", y_min = 0, y_max = 0.1)
pC <- make_plot(results_income, "income_group", "shortsleep",
                "% Reporting < 7 hrs of sleep",
                "", "D", show_legend = FALSE, legend_title = "Family Income", y_min = 0.2, y_max = 0.6)
pD <- make_plot(results_income, "income_group", "longsleep",
                "% Reporting > 9 hrs of sleep",
                "", "E", show_legend = TRUE, legend_title = "Family Income", y_min = 0, y_max = 0.1)
pE <- make_plot(results_age, "age_group", "shortsleep",
                "% Reporting < 7 hrs of sleep",
                "", "F", show_legend = FALSE, legend_title = "Age", y_min = 0.2, y_max = 0.6)
pF <- make_plot(results_age, "age_group", "longsleep",
                "% Reporting > 9 hrs of sleep",
                "", "G", show_legend = TRUE, legend_title = "Age", y_min = 0, y_max = 0.1)

fig2 <- (pA | pB) /
  (pC | pD) /
  (pE | pF)

ggsave('fig2_short_long_sleep_111225_v1.jpeg', fig2, device = jpeg, width = 15, height = 12)

# combine figures
combined <- (fig1 | fig2) + 
  plot_layout(widths = c(3, 5))

ggsave('combined_fig_120825_v1.jpeg', combined, device = jpeg, width = 16, height = 12)
